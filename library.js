...
Solr.deindexTopic = function (tid, callback) {
	topics.getPids(tid, function (err, pids) {
		if (err) {
			winston.error('[plugins/solr] Could not retrieve pids for deindexing');
			return callback(err);
		}
...
Solr.rebuildIndex = function (req, res) {
	...
	async.series({
		...
		if (err) {
			return winston.error('[plugins/solr] Could not retrieve base data for re-index');
		}
...
Solr.rebuildUserIndex = function (callback) {
	async.waterfall([
		...
		if (err) {
			winston.error('[plugins/solr] Could not retrieve users for user index rebuild');
			return callback(err);
		}
